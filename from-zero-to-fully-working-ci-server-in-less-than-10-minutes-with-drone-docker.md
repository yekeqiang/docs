...
#用 Drone 和 Docker 10 分钟搭建一个全功能的 CI 服务器

What is Drone? The official description is: "Drone is a Continuous Integration platform built on Docker".

什么是 [Drone](https://github.com/drone/drone) ？ 官方的解释是：“ Drone 是基于 Docker 构建的持续集成平台”。
...
什么是 [Docker](http://docker.io) ？官方的解释是：“ Docker 是一个容易创建的轻量级、易移植和对任何应用程序都是全功能容器的开源项目。 ”

***
这个描述Docker的请帮忙调整语序让它读起来更通顺一点。
***

10 minutes. This is all you need. In fact, there is an extra buffer in those 10 minutes to get your configuration working.

10 分钟，你只需要 10 分钟就。事实上，10 分钟外还需要额外的时间让你的配置工作起来。
...
10 分钟和让 [Jenkins](http://jenkins-ci.org) 服务启动和运行所需的时间比差了个数量级。我从来都不是 [Jenkins](http://jenkins-ci.org) 的粉丝，也许永远都不会是。还是让我们在其它文章中讨论这个问题吧。
...
我们假设你有一台可以访问的 64 位 Ubuntu 13.04 版本服务器。在 [Digital Ocean](http://digitalocean.com) (我非常喜欢 Digital Ocean，请使用我的[推荐链接](https://www.digitalocean.com/?refcode=9b3537dd733f)，谢谢～) 你可以使用预先安装好 Docker 版本的 VPS ，在写这篇文章的时候 Docker 的版本是 0.8 。

You can also use a Ubuntu 12.04 64 bit box in theory for Drone.

对于 Drone 理论上来说 64 位 Ubuntu 12.04 服务器也可以。
...
你的应用代码需要托管在 [Github](http://github.com) ，貌似 [Bitbucket](http://bitbucket.org) 很快就会支持。目前只支持 Github 对我来说没什么，我喜欢 Gibhub。

STEP 0: INSTALL DOCKER (OPTIONAL, SKIP IF DOCKER IS ALREADY INSTALLED)

第 0 步：安装 Docker (可选，如果你已经安装了 Docker 可以跳过这个步骤)
...
在 README 中有 “sudo start drone” 启动的不发，但是安装已经为我启动 Drone 。
...
现在你已经有一个 Drone 启动和运行了，但是我们必须配置 Github 的访问权限使得 Drone 在 Pull 或者 Push 请求后能够 Build 你的代码。
...
* 设定主页的地址是 http://my-server-ip-or-addr/
* 描述就像名字一样，对你有意义就可以。
* Github 认证的回掉地址是 http://my-server-ip-or-addr/auth/login/github


STEP 3: GET YOUR GITHUB CLIENT ID & SECRET CONFIGURED IN DRONE

第三步：获取 Github 的 Client ID 和 密钥 设置在 Drone 中
...
在 Github 注册完成新应用后，你会得到 Client ID 和密钥，把他们拷贝到 http://my-server-ip-or-addr/account/admin/settings 的 "GitHub OAuth Consumer Key and Secret" 部分。
...
点击  “New Repository”  按钮页面会转跳到 http://my-server-ip-or-addr/new/github.com 这里，点击 “Link Now” 按钮，接受 Repository Setup 页的配置，你会得到账户 ID 和所有代码仓库的详细信息。

Boom! Your project is almost ready to be built. You need only one final step now...

Boom！ 你的项目已经可以进行集成测试了，你只要完成最后一步。
...
这里有很多 Go、Python、Haskell、PHP、Scala、Node 的镜像。Drone 提供这些官方的镜像但是不限制使用使用其它的镜像。
...
可以在 [README](https://github.com/drone/drone#builds) 和 [drone.io](http://drone.io/) 的[文档](http://docs.drone.io/)中找到设置你的 .drone.yml 文件的说明。

When you are done, commit, push and watch the build. Rinse & repeat until your .drone.yml is working just fine.

当你完成这些后，commit，push 和 watch 的时候就会 Build 你的代码，反复重写你的 .drone.yml 也没有问题。 
...
就是这些，让你可以部署一个全功能的 CI 服务器。应该使用它替代 Jenkins、Codeship 或者其它的 CI 服务么？这完全取决于你。这个项目是一周前发布的，我还不能确定是否把它用在我工作的项目中，但是我会保持关注。
