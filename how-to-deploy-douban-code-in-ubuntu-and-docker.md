...
### Dependancies CODE required

- python 2.7 or up (Ubuntu inclusive)
...
There are two approaches to install. You can download the original libmemcached and Douban patch, then manually patch. Or you just download the libmemcached package Douban alrealy patched. As to the first approach, there is an error with appointed patch path (probably it's caused by libmemcached updates). I recommend to use the patched package directly.


首先，我们来安装必要的库和 g++ 编译器：

Firstly, we install necessary libary and g++ compiler:
...
Then download and unzip libmemcached package, compile and install:
...
Notice: You must complete the last step of "load config". During "make", You can specify the number of CUP cores is called, gengerally plus one more to the actual number. For example, your CPU is 4-core, then designat `make -j5` .
...
Although CODE provides scripts, there are some problems during execution. I reorganized the workflow based on my use. I use Ubuntu for sure. 
...
Scripts are [here](https://github.com/douban/code/tree/master/scripts). Each system has a corresponding script, currently supporting archlinux, centos, fedora, gentoo, openuse and ubuntu.
    
common.sh 脚本提供了一些通用的函数（如安装mysql、libmemcached ，如果已经安装好，则脚本会跳过这些软件的安
装）。
    
common.sh script provides some general fuctions (such as installing mysql and libmemcached, if they are installed, the scripts will skip installation).


OS_NAME.sh 根据每个系统的不同，调用了对应的安装命令和系统默认路径。基本过程是：

OS_NAME.sh compiles corresponding installation command and system default path. The process is:
...
Install mysql and corelative libaries:
...
Set up memecached port 11311 and restart memcachaed:
...
```
# 激活后，命令行的前面会加上（venv）
# After activation, (venv) will be add in front of command
$ virtualenv venv
$ . venv/bin/activate
```
...
If you use archlinx, install MySQL-python patch:
...
Install specified package in requirement.txt within CODE project:
...
＃Copy the materplate to viliya/local_config.py, CODE will read the configurations:
...
After vilya/local_config.py, you can read paremeters including domain, port, MySQL confi etc. If MySQL root user require password, add password on the 42nd raw.
...
At the last, launch CODE project (Notice: please replace 127.0.0.1 with a real IP):
...
Ok, CODE deployment is done. Open browser, input:
...
Register an account, and create a git library. Have FUN!
...
When you use pip to install python, it's too slow to visit abroad mirror, then you can use douban's mirrow (it's fast and solid with a full collection of all packages). Open `~/.pip/pip/conf` file (it probably dose not exist, you can edit and save), then input:
...
Well now, download python library from douban mirrow. Speedy!
...
It's quite easy to deploy CODE in Docker. Following Docker design philosophy, we seperate data and logic, create three different mirrors of MySQL, memcached and CODE, and communicate with each other. It's like you deploy them to three servers, but faster and easier. 

下面我们来看一下三个镜像的 Dockerfile ，先从 MySQL 开始：

Now we look up their Dockerfiles, from MySQL:
...
# Because Docker ubuntu mirror apt source is not adquate, we have to add
RUN    add-apt-repository -y "deb http://archive.ubuntu.com/ubuntu $(lsb_release -sc) universe"
RUN    apt-get --yes --force-yes update
RUN    apt-get --yes --force-yes upgrade
# 通过ENV指令，设定环境变量。将MySQL的root密码保存到变量
# Set up env variable via env command. Save MySQL root password to variable
$MYSQL_PASSWORD中
ENV    MYSQL_PASSWORD docker

# 安装MySQL过程中，要输入2次root密码
# During MySQL process, input root password twice
...
# Clone CODE project, we will use schema.sql to create valentine database
...
# Write "create database" into import.sh script, and add execute permission. Every RUN will commit a new mirrow, and lead to AUFS changes, therefore three import operations must be executed together.
...
# Set up permission for root user, and import database
RUN    /usr/sbin/mysqld & \
 sleep 10s &&\
...
EXPOSE 3306
# EXPOSE command specifies external interfaces. This is very important. By EXPOSE command, database's address and port in MySQL is open to CODE mirror. Then we can use Docker link to obtain address and port.

# CMD命令指定当镜像运行时，启动mysqld_safe
CMD ["/usr/bin/mysqld_safe", "--skip-syslog", "--log-error=/dev/null"]
# Start mysqld_safe CMD ["/usr/bin/mysqld_safe", "--skip-syslog", "--log-error=/dev/null"] when CMD specifies mirror to run
```

以上就是 MySQL 镜像的 Dockerfile ，只有短短的 20 行左右。下面我们再看 memcached 的 Dockerfile ：

Above is the Dockerfile of MySQL mirror, only 20 lines. Now let's check memcached Dokcerfile:
...
# Install build-essential library and g++ compiler wget
...
# replace memcached config, and set up port 11311
...
# 启动镜像时，启动memcached
# Start memcached when start mirrow
CMD memcached -u daemon
```

基本上就是 Ubuntu 下部署的命令，非常简单。最后我们再来看 CODE 镜像的 Dockerfile ：

It's similar to Ubuntu deployment command and very easy. 

Now it's the Dockerfile of CODE mirror:
...
# Install necessary softwares. Install file firstly. Lack of file will lead to Python magic library no found when gunicorn executed. 
...
# Each call RUN will produce a new layer, and AUFS path will change. So we write "Python database" into install.sh script. Add execute permission and execute
...
# Write "start CODE" to script. Add execute permission and execute
ADD ./start.sh /
RUN chmod +x start.sh

# 当启动时，执行start脚本
# Execute start script when start
CMD /start.sh
```

要从 Host 复制到镜像的 install.sh 脚本：
Copy install.sh script from Host

```
# 进入code目录并安装必要的python库
# Enter CODE directory and install necessary python library
cd code
pip install cython
...
# We obtain real-time ip of Docker mirror to replace the original
127.0.0.1
...
start.sh script start virtual environment and CODE.
...
Now we have three direcotries which are douban-mysql, douban-memcached and douban-code. We put three mirror Dockerfiles into corresponding directory, put install.sh and start.sh to douban-code directory, then construct mirror:
...
# -t parameter labels mirrors with douban/mysql
...
# continue constructing memcached
cd douban-code
...
It takes long while to compile memcached and keep patient. When three mirror are completely constructed, let's have a check:
...
# -d parameter lets mirror to run in daemon mode. If -d parameter is missed, mysql and memecached will be hung up.
# 镜像运行时候，我们用-name参数为容器指定一个便于访问的名称，这里就用mysql和memcached
# Us -name parameter to specify container name. In this case we use mysql and memecached
...
# -link parameter is in name:alias.
# -p参数对Host和Docker容器进行端口的重定向，也就是说，把Host的8812端口定向到CODE镜像的8000端口，这样，外部就可以通过8812端口访问CODE啦。
# -p parameter redirect Host and Docker container ports.
...
We don't specify -d parameter because we need to check the result of CODE run. When CODE mirror runs, created virtual environment and execute gunicor. When we see the output, open browser and input: 
